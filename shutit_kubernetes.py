...
class shutit_kubernetes(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.send('docker run --net=host -d kubernetes/etcd:2.0.5.1 /usr/local/bin/etcd --addr=$(hostname -i):4001 --bind-addr=0.0.0.0:4001 --data-dir=/var/etcd/data')
		shutit.send('docker run --net=host -d -v /var/run/docker.sock:/var/run/docker.sock gcr.io/google-containers/hyperkube:dev /hyperkube kubelet --api_servers=http://localhost:8080 --v=2 --address=0.0.0.0 --enable_server --hostname_override=127.0.0.1 --config=/etc/kubernetes/manifests')
		shutit.send('wget http://storage.googleapis.com/kubernetes-release/release/v0.14.1/bin/linux/amd64/kubectl')
		shutit.send('chmod +x kubelet')
		shutit.send('./kubectl version')
		shutit.send('./kubectl get pods')
		shutit.send('docker ps -a')
		...
		shutit.send_until('./kubectl get rc')
		shutit.send('./kubectl expose rc todopod --target-port=8000 --port=80')
		shutit.send('./kubectl get service')
		shutit.send('./kubectl resize rc todopod --replicas=2')
		shutit.send('./kubectl get pods')
		shutit.logout()
		shutit.logout()
		return True
